...
function AllAdmins() {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    if (result.success) {
      setRawAdmins((prev) => prev.filter((u) => u.uid !== selectedUser.uid));
  ...
  const handleChangeNameConfirm = useCallback(async (firstName, lastName) => {
    ...
    try {
      ...
      await logAdminAction({
        ...
        description: `Updated name for admin: ${selectedUser.email}`,
        details: {
  ...
  const handleChangeLocationConfirm = useCallback(async (locationData) => {
    ...
    try {
      ...
      await logAdminAction({
        ...
        description: `Updated location for admin: ${selectedUser.email}`,
        details: {
          userUid: selectedUser.uid,
          userEmail: selectedUser.email,
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      ...
      const adminsList = snapshot.docs.map((docSnap) => {
    ...
    setTableData({
      columns: [
        ...
        { Header: "Email", accessor: "email" },
        {
          ...
          accessor: "role",
          Cell: ({ value }) => (
            <Chip label={formatRole(value)} color={getRoleColor(value)} size="small" variant="outlined" />
          ),
        },
      ...
      <MDBox pt={6} pb={3}>
        <Card>
          <MDBox p={3} lineHeight={1}>
            <MDTypography variant="h5" fontWeight="medium">All Admins</MDTypography>
            <MDTypography variant="button" color="text">
              {rawAdmins.length} admin{rawAdmins.length !== 1 ? "s" : ""} found
            </MDTypography>
          </MDBox>
